...
def main(args):
    ...
    folder_name = f"{model_string_name}-{ckpt_string_name}-size-{args.image_size}-vae-{args.vae}-" \
                  f"cfg-{args.cfg_scale}-seed-{args.global_seed}-n_steps{args.n_steps}"
    ...
    for _ in pbar:
        ...
        model_fn = model_wrapper(
                ...
                condition=y,
                unconditional_condition=y_null,
        ...
        pil_samples = [Image.fromarray(sample) for sample in samples]
        inception_features = inception(pil_samples)
        for i, feature in enumerate(inception_features):
            index = i * dist.get_world_size() + rank + total
            save_pt = f"{sample_folder_dir}/{index:06d}.pt"
            torch.save({'inception_feature': feature.detach().cpu()}, save_pt)
